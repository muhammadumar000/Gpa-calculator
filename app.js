...
calculatedGpa.addEventListener("click", () => {
  ...
  for (let i = 0; i < Gpa.length; i++) {
    ...
    if ((Gpa[i] > 4 || Gpa[i] < 0) || (crdHrs[i] < 1 && crdHrs[i] > 4)) {
...
calc_internal.addEventListener("click", () => {
  ...
  if (!isNaN(internalMarks)) {
    ...
    if (internalMarks >= 45) {
      emoji = '🌟'; // Star for high GPA
    } else if (internalMarks >= 30 && internalMarks <= 45) {
      emoji = '🔥'
    }
    ...
    gpaOuput.innerHTML = `Marks: ${internalMarks} ${emoji}`;
